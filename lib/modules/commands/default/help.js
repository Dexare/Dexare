...
const keyValueForEach = (obj, func) => Object.keys(obj).map((key) => func(key, obj[key]));
    ...
    async run(ctx) {
        if (ctx.args.length) {
            ...
            if (!commands.length)
                ...
                let text = common_tags_1.stripIndents `
        ...
        `;
                ...
                if (command.metadata?.examples.length !== 0) {
                    ...
                    embed.fields.push({
                        name: 'Examples',
                        value: command.metadata.examples.map((a) => `${ctx.prefix}${a}`).join('\n'),
                        inline: true
                    });
                }
                const canSendEmbed = 'permissionsOf' in ctx.channel
        ...
        this.client.commands.commands.forEach((command) => {
            if (typeof command.hasPermission(ctx, ctx.event) === 'string')
                return;
            ...
            if (categories[category])
                ...
                categories[category] = [commandName];
        });
        ...
        keyValueForEach(categories, (cat, cmdNames) => {
            ...
            cmdNames.forEach((name) => {
                ...
                if (valueLength + length > 1024) {
                    fieldsPushed++;
                    embed.fields.push({
                        name: `${util_1.truncate(cat, 200)} (${fieldsPushed})`,
                        value: cmds.join(', ')
                    });
                    valueLength = 0;
                    cmds = [];
            ...
            embed.fields.push({
                name: fieldsPushed ? `${util_1.truncate(cat, 200)} (${fieldsPushed + 1})` : util_1.truncate(cat, 256),
                value: cmds.join(', ')
            });
        });
        // Chunk fields
        const chunkedFields = lodash_1.chunk(embed.fields, 4);
        chunkedFields.forEach((chunk, i) => embeds.push({
            ...embed,
            title: `Commands in ${ctx.guild ? util_1.truncate(ctx.guild.name, 100) : 'all servers'}${chunkedFields.length == 1 ? '' : ` (${i + 1})`}`,
            fields: chunk
        }));
        if (embeds.length === 1)
        ...
        try {
            ...
            if (ctx.guild)
                return 'Sent you a DM with information.';
        }
        catch (e) {
