...
const load_1 = __importDefault(require("./default/load"));
const unload_1 = __importDefault(require("./default/unload"));
const reload_1 = __importDefault(require("./default/reload"));
...
class CommandsModule extends module_1.default {
    ...
    registerDefaults(commands) {
        if (!commands)
            commands = ['eval', 'help', 'ping', 'exec', 'kill', 'load', 'unload', 'reload'];
        ...
        if (commands.includes('load'))
            this.register(load_1.default);
        if (commands.includes('unload'))
            this.register(unload_1.default);
        if (commands.includes('reload'))
            this.register(reload_1.default);
    }
    /** @hidden */
    ...
    async onMessage(event, message) {
        ...
        if ('permissionsOf' in message.channel && !ctx.guild.members.has(ctx.client.bot.user.id)) {
            ctx.guild.members.set(ctx.client.bot.user.id, (await ctx.guild?.fetchMembers({ userIDs: [ctx.client.bot.user.id] }))[0]);
        }
